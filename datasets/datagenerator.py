...
class ImageDataGenerator(object):
    ...
    def __init__(self, txt_file, root , mode, batch_size, num_classes, shuffle=True,
                 buffer_size=1000):
        ...
        if mode == 'training':
            data = data.map(self._parse_function_train, num_parallel_calls=8).prefetch(batch_size)
        elif mode == 'inference':
            data = data.map(self._parse_function_train, num_parallel_calls=8).prefetch(batch_size)
        else:
